...
bool InstrumentalRelabelingStrategy::relabel(
  ...
  //std::cout<< "average_treatment" << average_treatment << std::endl;
  //std::cout<< "average_instrument" << average_instrument << std::endl;  
  //std::cout<< "average_regularized_instrument" << average_regularized_instrument << std::endl;    
  ...
  for (size_t sample : samples) {
    ...
    double regularized_instrument = (1 - reduced_form_weight) * instrument
                                    + reduced_form_weight * treatment; //treatment
  ...
  for (size_t sample : samples) {
    ...
    double treatment = data.get_treatment(sample);
    double instrument = data.get_instrument(sample);
    double regularized_instrument = (1 - reduced_form_weight) * instrument + reduced_form_weight * treatment;//treatment
    ...
    //std::cout << "(" << sample << "=" << response << ":" << responses_by_sample(sample, 0) << ")" << std::endl;
  }
